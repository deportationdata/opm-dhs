...
agents <- map(DATA_TYPES, \(type) {
  files <- sort(list.files("data", pattern = glue("{type}-dhs-\\d{{6}}\\.parquet"), full.names = TRUE))
  ...
  expected <- seq(as.Date("2021-01-01"), latest_date, by = "month") |> format("%Y%m")
  ...
  inventory <- tibble(yyyymm = expected, present = expected %in% months_on_disk, lag = lag_months)

  inventory_agent <- inventory |>
    create_agent(label = glue("{type} — file inventory"), actions = action_levels(warn_at = 1, stop_at = 1)) |>
    col_vals_equal(columns = vars(present), value = TRUE, label = "no gaps in monthly series") |>
  ...
  content_agent <- df |>
    ...
    col_schema_match(
      schema = col_schema(.cols = setNames(rep("character", length(EXPECTED_COLS[[type]])), EXPECTED_COLS[[type]])),
      complete = FALSE, in_order = FALSE,
      label = "expected columns present and character type"
    ) |>
    col_vals_equal(columns = vars(agency), value = DHS_AGENCY, label = "agency == DHS") |>
